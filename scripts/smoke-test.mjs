...
assert.match(rootReadme, /To verify the FAQ review result without copying the wrapped JSON-RPC response/);
assert.match(rootReadme, /examples\/faq-candidate-review-json-rpc-session\/sample-session\.jsonl \| tail -n 1 \| node -e/);
assert.match(rootReadme, /recommendedStatus\); \}\)'/);
assert.match(rootReadme, /public_faq_candidate` for the synthetic pricing-plan question/);
assert.match(rootReadme, /question-pattern label, risk level, expected status, and returned\s+`recommendedStatus`/);
...
assert.match(triedItFeedbackTemplate, /tools\/list feedback shape/);
assert.match(triedItFeedbackTemplate, /visible tool\s+count/);
assert.match(triedItFeedbackTemplate, /visible tool names/);
assert.match(triedItFeedbackTemplate, /returned `error\.message` only/);
assert.match(triedItFeedbackTemplate, /version mismatch feedback shape/);
assert.match(triedItFeedbackTemplate, /`npm view` version/);
assert.match(triedItFeedbackTemplate, /`--help` header version/);
assert.match(triedItFeedbackTemplate, /`initialize` serverInfo\.version/);
assert.match(triedItFeedbackTemplate, /only a desktop MCP client appears\s+stale/);
